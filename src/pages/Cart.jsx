...
export default function Cart({ items, onRemove, onClear, onInc, onDec }) {
        ...
        <>
              ...
              <li key={product.id} className="flex items-center justify-between p-3">
                <div>
                  ...
                  <div className="text-sm opacity-70 flex items-center">
                    <button className="rounded-lg border px-3 py-1 text-sm hover:bg-black/5" onClick={() => onDec(product.id)}>−</button>
                    <span className="mx-2 min-w-6 text-center inline-block">{qty}</span>
                    <button className="rounded-lg border px-3 py-1 text-sm hover:bg-black/5" onClick={() => onInc(product.id)}>+</button>
                  </div>
                </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button className="rounded-xl border px-3 py-2 hover:bg-black/5" onClick={onClear}>Vaciar</button>
              <button className="rounded-xl bg-black px-3 py-2 text-white hover:opacity-90">Checkout</button>
            </div>
          </div>
        </>
      )}
